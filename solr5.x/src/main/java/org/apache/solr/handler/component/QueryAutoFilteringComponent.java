...
public class QueryAutoFilteringComponent extends QueryComponent
    ...
    private void addModifier(String modifierPhrase, String modifierFields) {
        ...
        if (modifierFields.indexOf(fieldDelim) > 0) {
            ...
            String fieldPairs =
                new String(modifierFields.substring(modifierFields.indexOf(fieldDelim) + 1));
            modifierFields =
                new String(modifierFields.substring(0, modifierFields.indexOf(fieldDelim)));
            Log.info("fieldPairs = " + fieldPairs);

            String modifierTemplate = null;
            if (fieldPairs.indexOf(fieldDelim) > 0) {
                modifierTemplate =
                    new String(fieldPairs.substring(fieldPairs.indexOf(fieldDelim) + 1));
            ...
            if (fieldPairs.indexOf(",") > 0) {
                ...
                for (int i = 0; i < fieldPairList.length; i++) {
                    String field =
                        new String(fieldPairList[i].substring(0, fieldPairList[i].indexOf(":")));
                    String value =
                        new String(fieldPairList[i].substring(fieldPairList[i].indexOf(":") + 1));
        ...
        if (initParams != null) {
            ...
            if (synonymsFile != null) {
                ...
                try {
                    ...
                    parser.parse(
                        new InputStreamReader(resourceLoader.openResource(synonymsFile), decoder));
    ...
    private boolean findPattern(ArrayList<char[]> queryTokens, ResponseBuilder rb,
        ModifiableSolrParams modParams) throws IOException {
        ...
        HashMap<String, int[]> entityPositionMap =
            (verbModifierList != null) ? new HashMap<String, int[]>() : null;
                ...
                if (indexedTerm != null) {
                    ...
                    if (indexedTerm.indexOf(fieldDelim) > 0) {
                        ...
                        for (int t = 0; t < indexedTerms.length; t++) {
                            ...
                            if (entityPositionMap != null)
                                entityPositionMap.put(indexedTerms[t], entityPosition);
                        }
                    } else {
                        valList.add(indexedTerm);
                        if (entityPositionMap != null)
                            entityPositionMap.put(indexedTerm, entityPosition);
                    }
            ...
            if (boostFactor == null) {
                ...
                if (qbuilder.length() == 0 && fieldMap.size() > 0) {
                    ...
                    for (String fieldName : fieldMap.keySet()) {
                        String fq = getFilterQuery(rb, fieldName, fieldMap.get(fieldName),
                            fieldPositionMap.get(fieldName), queryTokens, "");
                    ...
                    for (String fieldName : fieldMap.keySet()) {
                        if (boolQ.length() > 0)
                            boolQ.append(" AND ");
                        boolQ.append(getFilterQuery(rb, fieldName, fieldMap.get(fieldName),
                            fieldPositionMap.get(fieldName), queryTokens, ""));
                    }
                ...
                for (String fieldName : fieldMap.keySet()) {
                    bbuilder.append(" ");
                    bbuilder.append(getFilterQuery(rb, fieldName, fieldMap.get(fieldName),
                        fieldPositionMap.get(fieldName), queryTokens, boostSuffix));
                }
    ...
    private String getFilterQuery(ResponseBuilder rb, String fieldName, ArrayList<String> valList,
        ...
        if (fieldName.indexOf(fieldDelim) > 0) {
            return getFilterQuery(rb, fieldName.split(fieldSplitExpr), valList, termPosRange,
                queryTokens, suffix);
        }
        if (valList.size() == 1) {
            ...
            if (term.indexOf(" ") > 0) {
                ...
                if (singleTermQuery != null) {
                    ...
                    strb.append("(").append(fieldName).append(":").append(term).append(" OR (")
                        .append(singleTermQuery).append("))").append(suffix);
    ...
    private String getFilterQuery(ResponseBuilder rb, String[] fieldNames,
        ArrayList<String> valList, int[] termPosRange, ArrayList<char[]> queryTokens,
        String suffix) {
        StringBuilder filterQBuilder = new StringBuilder();
        for (int i = 0; i < fieldNames.length; i++) {
            if (i > 0)
                filterQBuilder.append(" OR ");
            filterQBuilder.append(
                getFilterQuery(rb, fieldNames[i], valList, termPosRange, queryTokens, suffix));
        }
        return "(" + filterQBuilder.toString() + ")";
    }

    private String getFieldNameFor(ArrayList<char[]> queryTokens, int startToken, int endToken)
        throws IOException {
            ...
            for (int i = 0; i < terms.length; i++) {
                ...
                if (fieldName.indexOf(fieldDelim) > 0) {
                    ...
                    for (int f = 0; f < fields.length; f++) {
                        if (f > 0)
                            strb.append(" OR ");
                        strb.append(fields[f]).append(":")
                            .append(getMappedFieldName(termMap, terms[i].toLowerCase()));
                    }
                    strb.append(")");
                } else {
    ...
    private String getFieldNameFor(String phrase) throws IOException {
        ...
        return ("*".equals(phrase) || "* *".equals(phrase)) ?
            null :
            getMappedFieldName(fieldMap, phrase.toLowerCase());
    }
            ...
            if (mappedFields.size() == 1) {
                ...
                for (String fieldName : mappedFields) {
                    if (fieldBuilder.length() > 0)
                        fieldBuilder.append(fieldDelim);
    ...
    private void buildFieldMap(ResponseBuilder rb) throws IOException {
        ...
        HashMap<String, UninvertingReader.Type> fieldTypeMap =
    ...
    private void addTerm(CharsRef fieldChars, String fieldValue, SynonymMap.Builder fieldBuilder,
        SynonymMap.Builder termBuilder) throws IOException {
    ...
    private ArrayList<String> getSynonymsFor(SynonymMap synMap, String term) throws IOException {
        ...
        for (int i = 0; i < tokens.length; i++) {
            ...
            if (i < tokens.length - 1
                && fst.findTargetArc(SynonymMap.WORD_SEPARATOR, scratchArc, scratchArc, fstReader)
                != null) {
    ...
    // ===========================================================================
    private void filterFieldMap(ArrayList<char[]> queryTokens,
        HashMap<String, ArrayList<String>> fieldMap, HashMap<String, int[]> entityPositionMap,
        HashMap<String, int[]> fieldPositionMap) {
        ...
        for (ModifierInstance modInstance : usedModifiers) {
            if (modInstance.templateRule != null)
                applyModifierTemplateRule(entityPositionMap, fieldMap, modInstance.templateRule);

            HashMap<String, String> fieldNameKeys =
                getFieldKeysForFieldName(modInstance.modifierFields, fieldMap);
            if (fieldNameKeys != null) {
                ...
                for (String fieldNameKey : fieldNameKeys.keySet()) {
                    ...
                    HashSet<String> entityPhrases =
                        findLastEntitiesBefore(entityPositionMap, modInstance, usedModifiers,
                            fieldMap.get(fieldNameKey));
                    if (entityPhrases != null) {
                        remapEntity(fieldNameKey, entityPhrases, modifierField, fieldMap,
                            fieldPositionMap, entityPositionMap);
                        remapped = true;
                    } else {
                        entityPhrases =
                            findFirstEntitiesAfter(entityPositionMap, modInstance, usedModifiers,
                                fieldMap.get(fieldNameKey));
                        if (entityPhrases != null) {
                            remapEntity(fieldNameKey, entityPhrases, modifierField, fieldMap,
                                fieldPositionMap, entityPositionMap);
            ...
            if (remapped && modInstance.filterFields != null) {
                ...
                for (String filtField : modInstance.filterFields.keySet()) {
                    ...
                    Log.info("setting verb filter: " + filtField + ":" + modInstance.filterFields
                        .get(filtField));
    ...
    private ArrayList<ModifierInstance> getOrderedModifierPositions(ArrayList<char[]> queryTokens) {
        ...
        while (i < queryTokens.size()) {
            ...
            if (modifier != null && matchesModifier(modifier.modTokens, queryTokens, i)) {
                ...
                Log.info("fields: ");
                for (String modField : modifier.modifierFields) {
                    Log.info("   " + modField);
                }
    ...
    private boolean matchesModifier(String[] modTokens, ArrayList<char[]> queryTokens, int start) {
        ...
        while ((start + i) < queryTokens.size() && i < modTokens.length) {
            ...
            if (!token.toLowerCase().equals(modTokens[i].toLowerCase()))
                return false;
            if (++i == modTokens.length)
                return true;
        }
        return false;
    }
    ...
    private HashMap<String, String> getFieldKeysForFieldName(ArrayList<String> modifierFields,
            ...
            for (String fieldNameList : fieldMap.keySet()) {
                ...
                for (int i = 0; i < fields.length; i++) {
                    if (fields[i].equals(modifierField)) {
                        if (fieldKeys == null)
                            fieldKeys = new HashMap<String, String>();
    ...
    private HashSet<String> findLastEntitiesBefore(HashMap<String, int[]> entityPositionMap,
        ModifierInstance modifier, ArrayList<ModifierInstance> usedModifiers,
        ArrayList<String> fieldVals) {
        ...
        for (String entityPhrase : entityPositionMap.keySet()) {
            ...
            if (fieldVals.contains(entityPhrase)) {
                ...
                Log.info(
                    "mod is at " + thisModPos + " previous mod was " + previousModifierPosition);
    ...
    private HashSet<String> findFirstEntitiesAfter(HashMap<String, int[]> entityPositionMap,
        ModifierInstance modifier, ArrayList<ModifierInstance> usedModifiers,
        ArrayList<String> fieldVals) {
    ...
    private void remapEntity(String fieldNameKey, HashSet<String> entityValues,
        String modifierField, HashMap<String, ArrayList<String>> fieldMap,
        HashMap<String, int[]> fieldPositionMap, HashMap<String, int[]> entityPositionMap) {
        ...
        if (allMatch) {
            if (fieldNameKey.equals(modifierField))
                return;
    ...
    private void applyModifierTemplateRule(HashMap<String, int[]> entityPositionMap,
        HashMap<String, ArrayList<String>> fieldMap, ModifierTemplateRule modifierRule) {
        ...
        for (String firstEntity : firstEntityList) {
            ...
            if (secondPos != null && (secondPos[0] == firstPos[1] + 1)
                && findEntityList(fieldMap, modifierRule.entity_2_field) != null) {
    ...
    private ArrayList<String> findEntityList(HashMap<String, ArrayList<String>> fieldMap,
        String entityField) {
    ...
    private int[] getPosArrayFor(ArrayList<String> entities,
        HashMap<String, int[]> entityPositionMap) {
    ...
    private class ModifierDefinition {
        ...
        // the field(s) that will be used like 'memberOfGroup_ss,groupMembers_ss'
        String[] modTokens;
    ...
    private class ModifierTemplateRule {
        ...
        ModifierTemplateRule(String templatePattern) {
            String leftSide =
                new String(templatePattern.substring(0, templatePattern.indexOf("=>")));
            String rightSide =
                new String(templatePattern.substring(templatePattern.indexOf("=>") + 2));
